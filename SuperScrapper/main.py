...
def report(): #사용자가 찾으려고하는 단어 (parameter=value에서 value) 가져오기
    # print(request.args) #주소의 argument를 출력
    # print(request.args.get('word')) #html의 form에서 지정해준 word argument를 request이용해 주소에서뽑아오기
    word = request.args.get('word') # = user가 입력한 검색값
    if word:
        word = word.lower()
